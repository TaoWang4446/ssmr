...
public class RoleController {
	...
	/**
	 * ModelMap modelMap:
	 * 数据模型:从控制器获取数据后,会装载数据到数据模型和视图中,然后将视图名称转发到视图解析器中
	 * 通过解析器解析后得到最终的视图(物理视图),最后将数据模型渲染到视图中,展示最终的结果给用户
	 *
	 * 之前一直用 MddelAndView 来定义视图,包括json视图,也用它来加载数据模型,
	 * MddelAndView 有 一个类型为ModelMap的属性
	 ...
	 *     public ModelMap() {
	 *     }
	 *  因此他可以存放各种键值对,为了进一步定义数据模型功能,spring还创建了类 ExtendedModelMap
	 *  public class ExtendedModelMap extends ModelMap implements Model {
	 *     public ExtendedModelMap() {
	 *     }
	 *  在次基础上派生了关于数据绑定的类-BindAwareModelMap,
	 *  public class BindingAwareModelMap extends ExtendedModelMap {
	 *     public BindingAwareModelMap() {
	 *     }
	 *  在控制器的方法中,可以把ModeAndVeiw  Model ModelMap 作为参数. 用来保存数据模型.
	 *
